...
echo "Stop the tasks too: herdr session stop $session_name"
exec "$bundle_dir/build/ttyd" -i "$ip" -p "$port" -I "$bundle_dir/web/index.html" -W -O -c "$(cat "$state_dir/credential")" -w "$work_dir" "$herdr_bin" --session "$session_name"
