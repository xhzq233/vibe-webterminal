...
| Panel Enter | 草稿非空时先粘贴草稿再回车；空框时只向终端发送回车 |
| ↑ / ↓ / → | 终端方向键 |
| Ctrl+C | 通常用于中断前台命令 |
| Clear | 发送 Ctrl+L；由当前终端程序处理 |
| Esc | 发送 Escape |
| 右上角全屏图标 | 点击请求网页全屏，再点退出；浏览器不支持或拒绝时会显示提示 |

在 Codex 中，可点 **Send** 把草稿放入 Codex 的输入区，确认后再点 **Enter**；也可直接点 Panel 的 **Enter**，一次完成粘贴和提交。Send 是粘贴，不会先清空终端里已有的输入。终端程序如何处理多行粘贴由该程序决定。

在终端上单指滑动会传给 Herdr，并保留惯性滚动；页面本身不会随手势平移。长按拖动可选中文字，双指轻点发送右键。Herdr 选中后的 `Copied` 表示已发出复制请求；点网页右下方 **Copy** 才会写入当前设备剪贴板。浏览器拒绝复制时，会提供可手动复制的文本。

全屏需要点击按钮触发，不能自动进入；是否允许由浏览器决定。iPad 浏览器可能在输入时退出全屏，网页无法强制保持。普通模式也会按当前浏览器可用高度显示终端。
